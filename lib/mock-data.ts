import { CTODProject, ExtractedField, FieldConfig } from './types';
...
export const defaultFields: FieldConfig[] = [
  ...
  },
  {
    ...
    type: 'percentage',
    required: true,
    automationMode: 'Human Only',
  },
];

const now = new Date().toISOString();

export const seedProjects: CTODProject[] = [
  {
    id: 'demo-project',
    name: 'NSCLC CTOD Demo',
    indication: 'Oncology',
    templateName: 'CTOD Standard Template',
    pdfCount: 2,
    stage: 'Configured',
    createdAt: now,
    updatedAt: now,
...
function sampleEvidence(name: string): string {
  const evidence: Record<string, string> = {
    'Study Title':
      'This phase II, multicenter, open-label trial evaluated efficacy and safety in metastatic NSCLC.',
    Country:
      'Patients were enrolled across 18 sites in the United States and Canada.',
    Dose:
      'Participants received pembrolizumab 200 mg every 3 weeks until progression.',
    ORR:
      'The objective response rate was 43% in the efficacy-evaluable population.',
  };
